...
def baba_aggro(baba_prices, order_id):

	toReturn = []

	if len(baba_prices["buy"]) > 0 and len(baba_prices["sell"]) > 0:
		...
		toReturn.append({"type": "add", "order_id": order_id, "symbol": "BABA", "dir": "BUY", "price": baba_buy_price+1, "size": baba_buy_size})

		toReturn.append({"type": "add", "order_id": order_id, "symbol": "BABA", "dir": "SELL", "price": baba_sell_price-1, "size": baba_sell_size})
